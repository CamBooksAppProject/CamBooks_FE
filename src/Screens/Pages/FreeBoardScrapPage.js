  ...
  const fetchCommentCount = async (postId) => {
    try {
      ...
      const res = await fetch(
        `${BASE_URL}/cambooks/general-forum/comment/count?postId=${postId}`,
        {
          headers: {
  ...
  const renderItem = ({ item }) => (
    ...
    >
      <View style={{ flexDirection: "column" }}>
        ...
        <Text style={styles.contentsFont} numberOfLines={2}>
          {item.content ?? ""}
        </Text>
  ...
  return (
    <View style={styles.container}>
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.emptyText}>데이터 없음</Text>}
      />
    </View>
  );
}
